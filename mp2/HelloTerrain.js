...
/**@global Flying velocity */
var velocity = 0.001
/**@global Quat rotation parameter**/
var rot = quat.create();
/**@gobal fog on/off switch **/
var fog = 0;
...
function setupShaders() {
  ...
  shaderProgram.vertexColorAttribute = gl.getAttribLocation(shaderProgram, "aDiffuseLightColor");
...
function tick() {
    ...
    draw();
    var movement = vec3.fromValues(viewDir[0] * velocity, viewDir[1] * velocity, viewDir[2] * velocity);
...
function keyPressFunc(event) {
    ...
    if(event.keyCode == "40" || event.keyCode == "83" ) {
      quat.setAxisAngle(rot, right, degToRad(-0.5));
    ...
    if(event.keyCode == "39" ||event.keyCode == "68") {
      quat.setAxisAngle(rot, viewDir, degToRad(0.5));
      vec3.transformQuat(up, up, rot);
    }
    ...
    if(event.keyCode == "81") {
      quat.setAxisAngle(rot, up, degToRad(0.5));
      vec3.transformQuat(viewDir, viewDir, rot);
    }
     if( event.keyCode == "69") {
      quat.setAxisAngle(rot, up, degToRad(-0.5));
    ...
    if(event.keyCode == "70") {
      fog = (fog + 1) % 2;
      gl.uniform1i(shaderProgram.uniformFog, fog);
      console.log(fog);
    }
}
